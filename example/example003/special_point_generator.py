...
for each_row in range(row):
    for each_col in range(column):
        ...
        x_0 = 0 + (6 / (column - 1)) * each_col
        y_0 = -3 + (6 / (row - 1)) * each_row
        z_0 = -10 + tol

        x_1 = 0 + (6 / (column - 1)) * each_col
        y_1 = -3 + (6 / (row - 1)) * each_row
        z_1 = 10 - tol
...
[fixed_type.InsertTuple(i, (0, 0, 1)) for i in range(point_number)]
