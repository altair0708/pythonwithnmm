...
[point_type.InsertValue(i, 1) for i in range(6)]
...
point_velocity.InsertTuple(2, (0, 0, -0.0001))
point_velocity.InsertTuple(3, (0, 0, 0.0001))
point_velocity.InsertTuple(4, (0, 0, 0.0001))
point_velocity.InsertTuple(5, (0, 0, 0.0001))

# loading point: loading force
point_force = vtkDoubleArray()
point_force.SetName('force')
point_force.SetNumberOfComponents(3)
[point_force.InsertTuple(i, (i, i, i)) for i in range(6)]

point_displacement = vtkDoubleArray()
point_displacement.SetName('displacement_total')
point_displacement.SetNumberOfComponents(3)
[point_displacement.InsertTuple(i, (0, 0, 0)) for i in range(6)]
