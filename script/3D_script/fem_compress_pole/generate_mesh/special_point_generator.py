...
for each_row in range(row):
    for each_col in range(column):
        # x = 0 + (1 / (column - 1)) * each_col + start_point[0]
        # y = 0 + (1 / (row - 1)) * each_row + start_point[1]
        # z_0 = 0.9 + start_point[2]
        # z_1 = 0.1 + start_point[2]
        x = 0 + tol + ((10 - tol) / (column - 1)) * each_col + start_point[0]
        y = 0 + tol + ((10 - tol) / (row - 1)) * each_row + start_point[1]
        z_0 = 10 - tol + start_point[2]
        z_1 = 0 + tol + start_point[2]
